...
# Настройка вывода Plotly для Colab
...
def download_usgs_csv(filename="global_earthquakes_10yrs_M4.5_2025-12-10.csv"):
    """Загружает или создаёт датасет"""
    if os.path.exists(filename):
        print(f"✅ Файл {filename} уже существует")
        return filename

    print("📥 Загружаем данные землетрясений...")
    try:
        ...
        with open(filename, "wb") as f:
            f.write(r.content)
        print(f"✅ Данные загружены в {filename}")
...
def create_dummy_csv(filename):
    ...
    print(f"✅ Синтетический датасет создан: {filename}")
...
def load_and_preprocess(csv_path, mag_threshold=MAG_THRESHOLD):
    """Загружает CSV и готовит данные"""
    df = pd.read_csv(csv_path)
    print(f"📊 Исходная форма датафрейма: {df.shape}")
...
print("\n" + "="*60)
print("🚀 ЗАПУСК ОБУЧЕНИЯ НА " + str(params["epochs"]) + " ЭПОХ")
print("="*60 + "\n")
...
try:
    ...
    print(f"⚠️ Не удалось посчитать ROC-AUC: {e}")
    auc = 0.0
...
# ===================== ГРАФИК 1: ТОЧНОСТЬ =====================
print("\n📊 График 1: Точность обучения и валидация")
...
fig_accuracy.add_trace(go.Scatter(
    ...
    mode='lines',
    name='Обучающая точность',
    line=dict(color='#2196F3', width=3),
    hovertemplate='<b>Эпоха %{x}</b><br>Точность обучения: %{y:.4f}<extra></extra>'
))
...
fig_accuracy.add_trace(go.Scatter(
    ...
    hovertemplate='<b>Эпоха %{x}</b><br>Валидационная точность: %{y:.4f}<extra></extra>'
))

fig_accuracy.update_layout(
    title='<b>Точность обучения и валидация</b>',
    xaxis_title='Эпоха',
...
fig_accuracy.show()
print("✅ График 1 выведен")

# ===================== ГРАФИК 2: ПОТЕРИ =====================
print("\n📊 График 2: Функция потерь обучения и валидация")
...
fig_loss.add_trace(go.Scatter(
    ...
    y=train_loss_smooth,
    mode='lines',
    name='Обучающая функция потерь',
    line=dict(color='#4CAF50', width=3),
    hovertemplate='<b>Эпоха %{x}</b><br>Потери обучения: %{y:.4f}<extra></extra>'
))
...
fig_loss.add_trace(go.Scatter(
    ...
    y=val_loss_smooth,
    mode='lines',
    name='Валидационная функция потерь',
    line=dict(color='#F44336', width=3),
    hovertemplate='<b>Эпоха %{x}</b><br>Валидационные потери: %{y:.4f}<extra></extra>'
))

fig_loss.update_layout(
    title='<b>Функция потерь обучения и валидация</b>',
    xaxis_title='Эпоха',
...
fig_roc.update_layout(
    title='<b>ROC-кривая для прогноза сильных землетрясений</b>',
...
fig_roc.show()
print("✅ График 3 выведен")

# ===================== СОХРАНЕНИЕ МОДЕЛИ =====================

model.save("earthquake_model.h5")
print("\n✅ Модель сохранена в earthquake_model.h5")
...
def predict_probability(lat, lon, depth, mag,
    ...
    """Функция предсказания вероятности сильного землетрясения"""
    values = []
...
examples = [
    (35.0, 140.0, 30.0, 5.5, "Слабое землетрясение в Японии"),
    (40.0, 120.0, 25.0, 6.5, "Сильное землетрясение в Китае"),
    (-15.0, -75.0, 100.0, 7.2, "Очень сильное в Южной Америке"),
]

for lat, lon, depth, mag, desc in examples:
    proba = predict_probability(lat=lat, lon=lon, depth=depth, mag=mag)
    status = "🔴 СИЛЬНОЕ" if proba > 0.5 else "🟢 СЛАБОЕ"
    print(f"{status}: {desc}")
...
def make_mag_hist():
    ...
    fig = px.histogram(
        ...
        x="mag",
        nbins=50,
        title="📊 Распределение магнитуд",
    )
    return fig

project_explanation = f"""
## 🌍 Нейросетевая модель прогноза землетрясений

### 📊 Параметры обучения:
- **Эпохи**: {params['epochs']}
- **Batch Size**: {params['batch_size']}
- **Learning Rate**: {params['learning_rate']}
- **Скрытые слои**: {params['hidden_layers']}
...
app.layout = dbc.Container(
    [
        html.H1("🌍 Система прогноза землетрясений", className="mt-5 mb-4"),
        html.Hr(),
        ...
        html.H3("📊 Ключевые метрики", className="mt-4 mb-3"),
        dbc.Row([
            dbc.Col([
                html.Div([
                    html.H5("🔴 Test Loss", style={"color": "#F44336"}),
            ...
            dbc.Col([
                html.Div([
                    html.H5("🟢 Test Accuracy", style={"color": "#4CAF50"}),
            ...
            dbc.Col([
                html.Div([
                    html.H5("🔵 ROC-AUC", style={"color": "#2196F3"}),
            ...
            dbc.Col([
                html.Div([
                    html.H5("📈 Эпохи", style={"color": "#FF9800"}),
        ...
        html.H3("📈 Графики обучения", className="mt-4 mb-3"),
        dbc.Row([
        ...
        html.H3("🔮 Интерактивный прогноз", className="mt-4 mb-3"),
        dbc.Row([
            dbc.Col([
                dbc.Label("📍 Широта", className="fw-bold"),
                dbc.Input(id="inp-lat", type="number", value=35.0),
                dbc.Label("📍 Долгота", className="fw-bold mt-3"),
                dbc.Input(id="inp-lon", type="number", value=140.0),
                dbc.Label("⬇️ Глубина (км)", className="fw-bold mt-3"),
                dbc.Input(id="inp-depth", type="number", value=30.0),
                dbc.Label("📊 Магнитуда", className="fw-bold mt-3"),
                dbc.Input(id="inp-mag", type="number", value=5.5),
            ], md=3),
            dbc.Col([
                dbc.Label("⚙️ Gap (опционально)", className="fw-bold"),
                dbc.Input(id="inp-gap", type="number", value=0.0),
                dbc.Label("📏 dmin (опционально)", className="fw-bold mt-3"),
                dbc.Input(id="inp-dmin", type="number", value=0.0),
                dbc.Label("⏱️ rms (опционально)", className="fw-bold mt-3"),
                dbc.Input(id="inp-rms", type="number", value=0.0),
            ], md=3),
            dbc.Col([
                dbc.Label("📊 horizontalError (опционально)", className="fw-bold"),
                dbc.Input(id="inp-herr", type="number", value=0.0),
                dbc.Label("📊 depthError (опционально)", className="fw-bold mt-3"),
                dbc.Input(id="inp-derr", type="number", value=0.0),
                dbc.Label("📊 magError (опционально)", className="fw-bold mt-3"),
                dbc.Input(id="inp-merr", type="number", value=0.0),
            ], md=3),
            dbc.Col([
                html.Div([
                    dbc.Button("🔍 Рассчитать", id="btn-predict", color="primary", size="lg", className="w-100 mt-4"),
...
def update_prediction(n_clicks, lat, lon, depth, mag, gap, dmin, rms, herr, derr, merr):
    ...
    status = "🔴 СИЛЬНОЕ ЗЕМЛЕТРЯСЕНИЕ!" if proba > 0.5 else "🟢 Слабое землетрясение"
    return f"{status}\nВероятность: {proba:.4f} ({proba*100:.2f}%)"
